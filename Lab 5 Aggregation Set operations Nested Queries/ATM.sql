...
--2)
Select city,Count(userId) As TotalUsers from [User] Group By city
--3)
Select city,Count(userId) As TotalUsers from [User] Group By city order by Count(userId)  desc
--4)
Select U.name,COUNT(UC.userID) AS No_Of_Cards from [user] U inner join UserCard UC on U.userID = UC.userID group by U.name Order by COUNT(UC.userID)
--5)
select cardNum,COUNT(cardNum) AS [No Of Transactions] from [Transaction]
Group by cardNum
having COUNT(CardNum)>2
--6
Select * from [Card]
Select * from [Transaction]
Select cardNum from [Card] C where NOT EXISTS(Select cardNum from [Transaction] T where T.cardNum = C.cardNum)

--7)
Select *
from [User] AS U where U.userId in (select UC.userId
									from UserCard AS UC where UC.cardNum in 
									(select cardNum from [Card] C where C.balance = (select  MAX([Card].balance) from [Card])
									) )
--8)
select * from [User] where usertype=(select userType from [User] Except (select name from UserType))  
--9)
